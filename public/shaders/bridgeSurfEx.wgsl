...
fn vMain(vsInput: VertexInput) -> VertexOutput {
    ...
    if ((hPosition.x >= mMin.x && hPosition.x <= mMax.x) && (hPosition.y >= mMax.y && hPosition.y <= mMin.y)) {
        ...
        highZ = dynamicUniform.exaggeration * altitude2Mercator(latFromMercatorY(hPosition.y), 0.2) * 10.0;
        lowZ = dynamicUniform.exaggeration * altitude2Mercator(latFromMercatorY(lPosition.y), 0.2) * 10.0;
    } else {
