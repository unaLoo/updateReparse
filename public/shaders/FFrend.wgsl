...
@group(2) @binding(3) var transformHighTexture: texture_2d<f32>;
@group(2) @binding(4) var transformLowTexture: texture_2d<f32>;
@group(3) @binding(5) var projectionTex: texture_2d<f32>;
...
fn sampleGeoPosition(pos: vec2f) -> vec2f {
    ...
    let textureSize = vec2i(textureDimensions(transformHighTexture, 0).xy);
    ...
    let geoPos0 = translateRelativeToEye(highGeoPos0, vec2f(0.0));
    let geoPos1 = translateRelativeToEye(highGeoPos1, vec2f(0.0));
    let geoPos2 = translateRelativeToEye(highGeoPos2, vec2f(0.0));
    let geoPos3 = translateRelativeToEye(highGeoPos3, vec2f(0.0));
