...
fn vMain(vsInput: VertexInput) -> VertexOutput {
    ...
    // let triangleIndex = vsInput.vertexIndex / 3;
    // let head = triangleIndex / 2;
    // var vertexID: u32;

    // // Up triangle
    // if (triangleIndex % 2 == 0) {

    //     vertexID = select(head, head + 1, vsInput.vertexIndex - triangleIndex * 3 > 1);
    // } else {

    //     vertexID = select(head - 1, head, vsInput.vertex - triangleIndex * 3 > 0);
    // }
    ...
    if ((hPosition.x >= mMin.x && hPosition.x <= mMax.x) && (hPosition.y >= mMax.y && hPosition.y <= mMin.y)) {
        ...
        highE = mix(centerUniform.eB[0], centerUniform.eT[0], elevation);
        lowE = mix(centerUniform.eB[1], centerUniform.eT[1], elevation);
        ...
        // z = dynamicUniform.exaggeration * altitude2Mercator(latFromMercatorY(hPosition.y), elevation) * 10.0;
        // z = dynamicUniform.exaggeration * elevation / 1000000.0;
        highZ = select(highZ, 0.000000001, highZ > 0.0);
        lowZ = select(lowZ, 0.000000001, lowZ > 0.0);
        // z = select(z, 0.000000001, z > 0.0);
    } else {
        
        z = 0.0;
    }

    highZ = select(highZ, 0.000000001, vsInput.vertexIndex % 2 == 0);
    lowZ = select(lowZ, 0.000000001, vsInput.vertexIndex % 2 == 0);

    let correctPos = translateRelativeToEye(vec3f(hPosition, highZ), vec3f(lPosition, lowZ));
...
fn fMain(fsInput: VertexOutput) -> @location(0) vec4f {
    ...
    // return vec4f(vec3f(0.8), 0.5);
    return vec4f(vec3f(170.0, 170.0, 187.0) / 255.0, 0.3);
}